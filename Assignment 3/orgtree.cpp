...
bool Orgtree::isEmployeePresentInOrg(Employee* head, int e_id) {
  ...
  // terminate loop if currentEmployee = null
  while(currentEmployee) {
      // Check if currentEmployee is the searched for
      if (currentEmployee->getEmployeeID() == e_id) {
          return true;
      }

      //
      else if (e_id < currentEmployee->getEmployeeID()){
